...
### 与TCP/IP融合 ###
由于应用角度的差异，云计算中心、数据中心的应用程序并不能直接运行在高性能平台上。两者编程思路和编程接口差异很大。目前的数据中心和云计算中心大量使用IP应用程序，采用Socket等技术，基于TCP/IP及其衍生的应用层协议栈实现。具有代表性的IP应用程序如：基于HTTP协议的浏览器和网络应用,基于FTP协议的文件分享站点,基于SMTP、POP3协议的邮件应用以及常用的telnet,DNS服务等。
