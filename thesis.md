...
#### 数据缓冲区结构sk_buff ####
TCP/IP在Linux中的实现，依赖一个重要的数据结构sk_buff。在TCP/IP协议中，数据包需要在应用层、传输层、网基层和链路层之间传递。每一层次对数据包进行增加对应头部（发送数据包的情况）或是删除对应头部（接收数据包的情况）的处理，有时称为封包和拆包。Linux内核使用sk_buff统一表示正处于不同层次上的数据包。实质上，sk_buff是一个缓冲区的控制结构。

![sk_buff配图]()

描述图！！

我们关注对sk_buff指向的缓冲区可以执行的五种操作：分配缓冲区（alloc_skb）、添加协议头部数据（skb_put）、添加协议尾部数据（skb_push）、去除协议头部数据（skb_pull）和保留数据头部空间（skb_reserve）。这些处理过程分别如图[]所示。
