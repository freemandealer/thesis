...
### 融合互连网络的通信的需求 ###
#### 融合互连网络通信系统 ####
针对二者融合的趋势，中国科学院计算技术研究所设计实现了高性能融合互连网络通信系统，简称融合互连网络通信系统。融合互连网络通信系统是一套软硬件结合的系统架构，其系统架构设计如图[]所示。

![融合互连网络通信系统]()

一个完整的通信系统包括两部分，一部分是通信硬件，另一部分是通信软件。通信硬件分为两层，分为融合互连网络接口控制器（简称融合控制器）和硬件接口。而通信软件分为两部分，其中一部分是宿主机部分的通信软件，另一部分是虚拟机内的通信软件。它们共同包括驱动和软件通信接口。通信系统在通信软件和通信硬件的协同设计下为上层的应用程序提供支撑，来达到最大的通用性和I/O通信性能。

#### TCP/IP通信的需求 ####
然而由于应用角度的差异，云计算中心、数据中心的应用程序并不能直接运行在高性能平台上。高性能计算机能计算机的编程思路和编程接口也和普通应用角度不同应用程序不同。目前的数据中心和云计算中心的应用程序，大多采用Socket等技术，基于TCP/IP及其衍生的应用层协议栈实现。代表性的如：基于HTTP协议的浏览器和网络应用,基于FTP协议的文件分享站点,基于SMTP、POP3协议的邮件应用以及常用的telnet,DNS服务等。而高速的外网接口和TCP/IP支持对用于计算密集场景的高性能计算机都不是必须的。

如何在不修改上层应用程序的情况下，让需要TCP/IP支持的应用程序运行在使用融合互连网络通信系统的高性能平台上。这就是本文需要解决的问题。
